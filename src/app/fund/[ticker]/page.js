...
export async function generateMetadata({ params }) {
  ...
  if (!security) {
    return { title: `${ticker.toUpperCase()} Unit Trust - Not Found` };
  }

  const formattedCap = formatMarketCap(security.market_cap);

  return {
    title: `${security.symbol} Unit Trust - Price & Details - KLSE MarketCap`,
    description: `${security.name} (${security.symbol}) unit trust with AUM of ${formattedCap}. View price, charts, and details on KLSE MarketCap.`,
    openGraph: {
      title: `${security.symbol} Unit Trust - ${security.name}`,
...
export default async function FundDetailPage({ params }) {
  ...
  if (!security) {
    ...
    if (alias) {
      redirect(`/fund/${alias.new_symbol.toLowerCase()}`);
    }
  ...
  const breadcrumbs = [
    { label: 'Unit Trusts', href: '/funds' },
    { label: security.symbol },
  ];

  const fundJsonLd = generateEtfJsonLd(security, etfDetail);
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    { name: 'Home', href: '/' },
    { name: 'Unit Trusts', href: '/funds' },
    { name: security.symbol, href: `/fund/${security.symbol.toLowerCase()}` },
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: safeJsonLd(fundJsonLd) }}
      />
      <script
        type="application/ld+json"
      ...
      <div className="mt-2 space-y-8">
        ...
        <PriceChart initialData={dailyPrices || []} symbol={security.symbol} etfHistory={etfHistory || []} keyMetrics={keyMetrics} securityType="fund" />
